...
#IF RUNNING RANDOMIZED GRID SEARCH CV, replace above rf, metrics, results, and predictions with code below
#          # Train a Random Forest model
...
#             # Train a Random Forest model
#             #model
#         model = RandomForestRegressor(bootstrap = True, random_state = random_state)
#         clf = RandomForestRegressor(bootstrap = True, random_state = random_state)

#         # Arrays to store scores
#         #nested_scores = np.zeros(NUM_TRIALS)

#         ######
#         # Choose cross-validation techniques for the outer loop, independently of the dataset.
#         outer_cv = RepeatedKFold(n_splits=5, n_repeats=3, random_state=random_state)       
#         clf = RandomizedSearchCV(estimator=model, param_distributions=p_grid, n_iter = 40, cv=outer_cv, scoring='neg_mean_absolute_error')
...
#         nested_best_score = clf.best_score_
#         nested_params = clf.best_params_
#         nested_est = clf.best_estimator_

#         print('nested_best_score:\n', nested_best_score)
#         print('nested_params:\n', nested_params)
#         print('nested_est:\n', nested_est)
...
#               'Test': test,
#               'RandomState' : round(random_state),
#               'Nested Best Score': round(nested_best_score, 2),
#               'nested_params': nested_params,
#               'nested_est': nested_est})#,
#    return results
